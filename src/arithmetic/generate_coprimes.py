...
def write_to_file(data, primitiveroots):
	...
	for c in classes:
		if c != 63:
			f.write("#ifdef CKKS_ENGINE_MODE\n")
	...
	f.write("std::map<uint64_t, std::map<int, GaussianInteger>> NTHROOT = {\n")
	for i in coprimes:
		if ceil(log(i,2)) != 63:
			f.write("#ifdef CKKS_ENGINE_MODE\n")
		f.write("\t{(uint64_t)%d, {\n" % i)
	...
	f.write("std::map<uint64_t, std::map<int, GaussianInteger>> INVNTHROOT = {\n")
	for i in coprimes:
		if ceil(log(i,2)) != 63:
			f.write("#ifdef CKKS_ENGINE_MODE\n")
		f.write("\t{(uint64_t)%d, {\n" % i)
...
if __name__ == "__main__":
	...
	with open("coprimes.cu","w+") as f:
		...
		f.write("#include <cuPoly/settings.h>\n\n")
