...
__host__	void call_get_zo_sample(
	 ...
	 * Generate values
	 */
	memset(ctx->h_aux, 0, poly_get_residues_size(p->base));
...
__global__ void sample_from_uniform(
	...
	if (tid < N){	
		/* Copy state to local memory for efficiency */
		// 2^62-1
		//const uint64_t uppbound = 0x7FFFFFFFFFFFFFFF;
		// 2^32-1
		const uint64_t uppbound = 0XFFFFFFFF;

		curandState localState = states[tid];
		int64_t value1 = (int64_t) floor(curand_uniform_double(&localState)*uppbound);
		int64_t value2 = (int64_t) floor(curand_uniform_double(&localState)*uppbound);

		for(int rid = 0; rid < nresidues; rid++){
			GaussianInteger xi;
			xi.re =  value1 % coprimes[rid];
			xi.imag =  value2 % coprimes[rid];
