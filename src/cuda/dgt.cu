...
__host__ __device__ uint64_t mod(uint128_t a, int rid){
  ...
  #else
  uint64_t q = (mul64hi(m1, MU) << (64-numbits)) + ((m1 * MU) >> numbits);

  uint64_t q0 = q*p & z;
  uint64_t q1 = (mul64hi(q, p) << (64-numbits)) + ((q * p) >> numbits);
  #endif
...
// 
// [ RUN      ] cuPolyInstantiation/TestRNS.SimpleBasisExtensionUniformQtoQB/256_q120
...
// /home/pedro/spogpack/cupoly/tests/unit_tests/test_bfv_mode.cu:867: Failure
